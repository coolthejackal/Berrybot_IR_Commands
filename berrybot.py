...
class LEDMatrix:
    ...
    def tick(self, timer):
        ...
        self.set_columns(self.led_array_buffer[self.led_row])
        machine.Pin(self.row_pins[self.led_row]).value(1)
        # time.sleep_ms(1) satırı SİLİNDİ. Bu, timer çatışmasını çözer.   
...
class WS2812():
    def __init__(self, num_leds=1, pin_num=6, brightness=0.2):
        ...
        self.pixel_interval=120
        self.pixel_number = 0
        self.pixel_mode = 1
        self.chase_led_index = 0
        self.color_offset = 0
        self.rgb_value = [[255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0], [255, 0, 255], [0, 255, 255],[255, 127, 0]]
    ...
    def startup_animation(self, duration=200):
        red = [255, 0, 0]
        for cycle in range(3):
            for i in range(self.num_leds):
                self.pixels_set(i, red)
            self.pixels_show()
            time.sleep_ms(duration)
            for i in range(self.num_leds):
                self.pixels_set(i, (0, 0, 0))
            self.pixels_show()
            time.sleep_ms(duration)
    ...
    def pixel_function(self):
        # Kesintisiz chase: sönük bir ara faz olmadan bir LED sönerken sıradaki LED yanar.
        self.pixels_fill((0, 0, 0))
        color_index = (self.chase_led_index - self.color_offset) % self.num_leds
        color = self.rgb_value[color_index]
        self.pixels_set(self.chase_led_index, (color[0], color[1], color[2]))
        self.pixels_show()

        self.chase_led_index += 1
        if self.chase_led_index >= self.num_leds:
            self.chase_led_index = 0
            self.color_offset = (self.color_offset + 1) % self.num_leds
...
class TB6612():
    def __init__(self, AIN1, AIN2, BIN1, BIN2, PWMA, PWMB):
        ...
        self._last_lr = (None, None)
        self._last_log_ms = 0
        self._log_repeat_ms = 700
        self._log_min_interval_ms = 140
    ...
    def _log_move_if_needed(self, direction, speed):
        now = time.ticks_ms()
        changed = (direction != self._last_direction) or (speed != self._last_speed)
        due = time.ticks_diff(now, self._last_log_ms) >= self._log_repeat_ms
        interval_ok = time.ticks_diff(now, self._last_log_ms) >= self._log_min_interval_ms
        if (changed and interval_ok) or due:
            print("[MOTOR] move dir={} speed={}".format(self._dir_name(direction), speed))
            self._last_direction = direction
            self._last_speed = speed
            self._last_log_ms = now

    def _log_lr_if_needed(self, left_speed, right_speed):
        now = time.ticks_ms()
        changed = (left_speed, right_speed) != self._last_lr
        due = time.ticks_diff(now, self._last_log_ms) >= self._log_repeat_ms
        interval_ok = time.ticks_diff(now, self._last_log_ms) >= self._log_min_interval_ms
        if (changed and interval_ok) or due:
            print("[MOTOR] setMotorSpeed L={} R={}".format(left_speed, right_speed))
            self._last_lr = (left_speed, right_speed)
            self._last_log_ms = now
        
    def setMotorSpeed(self, left_speed, right_speed):
        self._log_lr_if_needed(left_speed, right_speed)
        if right_speed > 0:
    ...
    def move(self, direction, speed):
        ...
        self._log_move_if_needed(direction, speed)
